...
def test_invalid_yaml_raises_value_error(tmp_path):
  ...
  pytest.raises(
    ValueError,
    lambda: ProcessPipeline(config_path=bad_yaml, output_dir=tmp_path / "output", inputs={}, framework_config=None),
  )
...
def test_missing_required_config_keys_raises_value_error(tmp_path):
  ...
  pytest.raises(
    ValueError,
    lambda: ProcessPipeline(config_path=yaml_path, output_dir=tmp_path / "output", inputs={}, framework_config=None),
  )
...
def test_duplicate_deliverable_raises_value_error(tmp_path):
  ...
  cfg["PipelineSteps"] = [
    {"DisplayId": "step1", "ProcessStep": "dummy1", "Deliverables": {"out1": "some_value"}},
    {
      "DisplayId": "step2",
      "ProcessStep": "dummy2",
